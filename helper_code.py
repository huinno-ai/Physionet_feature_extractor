...
def find_challenge_files(data_directory):
    ...
    for ff in os.listdir(data_directory):

        if 'gz' in ff:
            continue
        # if 'CPSC' not in ff:
        #     continue
        # if 'ShaoxingUniv' not  in ff:
        #     continue
        # if 'XL' in ff:
        #     continue
        print(ff)
        for f in os.listdir(data_directory+ff):
            root, extension = os.path.splitext(f)
            if not root.startswith('.') and extension=='.hea':
                header_file = os.path.join(data_directory+ff, root + '.hea')
                recording_file = os.path.join(data_directory+ff, root + '.mat')
